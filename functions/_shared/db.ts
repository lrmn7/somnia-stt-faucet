...
export async function ensureSchema(db: D1Database): Promise<void> {
  ...
  try {
    ...
    await db.batch([
      ...
      db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS idx_claims_idempotency ON claims(idempotency_key)`),
      db.prepare(`CREATE INDEX IF NOT EXISTS idx_claims_wallet_created ON claims(wallet_hash, created_at DESC)`),
      db.prepare(`CREATE INDEX IF NOT EXISTS idx_claims_ip_created ON claims(ip_hash, created_at DESC)`),
      db.prepare(`CREATE INDEX IF NOT EXISTS idx_claims_status_created ON claims(status, created_at DESC)`),
      db.prepare(`CREATE TABLE IF NOT EXISTS daily_budget (
